...
class MockProfile:
    def __init__(self, user_id: str, email: str, username: str, first_name: str, last_name: str, is_active: bool = True):
...
class MockUserCrud:
    async def get_profile_by_user_id_from_db(self, db: Any, user_id: str) -> Optional[MockProfile]:
...
async def create_admin_profile(db: MockAsyncSession, admin_supa_user: MockSupabaseUser) -> bool:
    ...
    if not existing_profile:
        print(f"Local profile for admin user {admin_supa_user.id} not found, creating...")
...
async def run_test():
    ...
    try:
        existing_profile = await mock_user_crud.get_profile_by_user_id_from_db(test_db, test_id)
