...
async def test_direct_connection():
    ...
    try:
        ...
        params = await conn.fetchrow('SHOW ALL')
        print("\nServer parameters:")
        for key, val in params.items():
            if any(kw in key for kw in ['conn', 'pool', 'timeout', 'statement', 'idle']):
                print(f"  {key} = {val}")
...
async def test_sqlalchemy_connection():
    ...
    try:
        ...
        import urllib.parse
        base_url = f"postgresql+asyncpg://{conn_params['username']}:{conn_params['password']}@{conn_params['host']}:{conn_params['port']}/{conn_params['dbname']}"
