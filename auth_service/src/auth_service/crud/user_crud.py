...
async def get_profile_by_user_id_from_db(db_session: AsyncSession, user_id: UUID) -> Profile | None:
    ...
    try:
        ...
        logger.error(f"Database error while fetching profile for user_id {user_id}: {e}", exc_info=True)
        # Depending on desired error handling, could raise or return None
        return None
    except Exception as e:
        logger.error(f"Unexpected error while fetching profile for user_id {user_id}: {e}", exc_info=True)
        # Depending on desired error handling, could raise or return None
        return None
...
async def create_profile_in_db(db_session: AsyncSession, profile_in: ProfileCreate) -> Profile | None:
    ...
    try:
        ...
        user_id_as_uuid = UUID(str(profile_in.user_id)) if not isinstance(profile_in.user_id, UUID) else profile_in.user_id
        ...
        logger.info(f"DEBUG crud.create_profile_in_db: new_profile.email='{new_profile.email}', type='{type(new_profile.email)}' BEFORE add")
        ...
        logger.error(f"Database error during profile creation for user_id {profile_in.user_id}: {e}", exc_info=True)
        await db_session.rollback()
        return None
    except Exception as e:
        logger.error(f"Unexpected error during profile creation for user_id {profile_in.user_id}: {e}", exc_info=True)
        await db_session.rollback()
        return None

async def update_profile_in_db(db_session: AsyncSession, profile_id: UUID, profile_in: ProfileUpdate) -> Profile | None:
    """
    ...
    try:
        result = await db_session.execute(
            select(Profile).filter(Profile.id == profile_id) # Assuming Profile has an 'id' primary key
        )
        ...
        logger.error(f"Database error during profile update for profile_id {profile_id}: {e}", exc_info=True)
        await db_session.rollback()
        return None
    except Exception as e:
        logger.error(f"Unexpected error during profile update for profile_id {profile_id}: {e}", exc_info=True)
        await db_session.rollback()
        return None
