...
from auth_service.schemas.user_schemas import UserLoginRequest, MagicLinkLoginRequest, MagicLinkSentResponse, SupabaseUser, PasswordResetRequest, PasswordUpdateRequest, OAuthProvider, OAuthRedirectResponse, SupabaseSession, ProfileResponse # Added for login, magic link, logout, password reset, and OAuth tests
...
async def test_password_update_successful(
    ...
    try:
        ...
        # The jwt argument is no longer explicitly passed, as the supabase client
        # uses the session established by get_current_supabase_user
        # assert "jwt" in call_args.kwargs # This assertion is removed
...
@pytest.mark.asyncio
async def test_oauth_login_initiate_google_ok(
    async_client: AsyncClient, monkeypatch
):
    """
    ...
    mock_supabase_auth.sign_in_with_oauth = AsyncMock(
        return_value=MagicMock(url=expected_auth_url, state="mocked_oauth_state_value") # Provide state from the beginning
    )
    ...
    mock_app_settings.OAUTH_STATE_COOKIE_NAME = "test_pa_oauth_state"  # Use a distinct name for testing if desired
    mock_app_settings.OAUTH_STATE_COOKIE_MAX_AGE_SECONDS = 300
    mock_app_settings.ENVIRONMENT = "development" # To match secure=False for cookie in dev
    
    original_get_app_settings = app.dependency_overrides.get(user_auth_routes.get_app_settings)
    app.dependency_overrides[user_auth_routes.get_app_settings] = lambda: mock_app_settings

    try:
        # Make the request ONCE
        response = await async_client.get(f"/auth/users/login/{provider.value}", follow_redirects=False)
        ...
        assert state_cookie_value == "mocked_oauth_state_value" # Check against the state provided by the mock
        ...
        if original_get_app_settings:
            app.dependency_overrides[user_auth_routes.get_app_settings] = original_get_app_settings
        else:
...
async def test_oauth_callback_google_successful_new_user(
    ...
    mock_supa_user = create_mock_supa_user(id_val=mock_user_id, email=mock_email, confirmed=True)
    mock_supa_session = create_mock_supa_session(user=mock_supa_user)

    mock_supabase_auth = AsyncMock()
    mock_supabase_auth.exchange_code_for_session = AsyncMock(return_value=mock_supa_session)
    mock_supabase_auth.get_user = AsyncMock(return_value=MagicMock(user=mock_supa_user)) # Simulate user after exchange
    ...
    monkeypatch.setattr(user_auth_routes.user_crud, "get_profile_by_user_id_from_db", mock_get_profile_db)
    
    mock_newly_created_profile = MagicMock(spec=Profile) # Simulate the ORM model instance
    ...
    monkeypatch.setattr(user_auth_routes.user_crud, "create_profile_in_db", mock_create_profile_db)
    ...
    try:
        response = await async_client.get(
            ...
            cookies={mock_app_settings.OAUTH_STATE_COOKIE_NAME: state} # Send the state cookie
        )
        ...
        mock_supabase_auth.exchange_code_for_session.assert_called_once_with(auth_code=auth_code)
        mock_get_profile_db.assert_called_once_with(db_session=db_session_for_crud, user_id=str(mock_user_id))
        ...
        for header_name, header_value in response.headers.multi_items():
            if header_name.lower() == 'set-cookie':
                if mock_app_settings.OAUTH_STATE_COOKIE_NAME in header_value and "Max-Age=0" in header_value:
                    found_cleared_cookie = True
                    break
        assert found_cleared_cookie, f"OAuth state cookie '{mock_app_settings.OAUTH_STATE_COOKIE_NAME}' not cleared."
...
async def test_oauth_callback_google_provider_error(
    ...
    mock_supabase_client_instance = AsyncMock()
    mock_supabase_client_instance.auth = mock_supabase_auth
    async def mock_get_supabase_override(): return mock_supabase_client_instance
    original_get_supabase = app.dependency_overrides.get(real_get_supabase_client)
    app.dependency_overrides[real_get_supabase_client] = mock_get_supabase_override
...
async def test_oauth_callback_google_invalid_code(
    ...
    mock_supabase_auth = AsyncMock()
    mock_supabase_auth.exchange_code_for_session = AsyncMock(
        side_effect=AuthApiError(message="Invalid grant: authorization code is invalid", status=400, code="invalid_grant")
    )
    mock_supabase_client_instance = AsyncMock()
    mock_supabase_client_instance.auth = mock_supabase_auth
    async def mock_get_supabase_override(): return mock_supabase_client_instance
...
async def test_oauth_callback_google_invalid_state(
    async_client: AsyncClient,
    monkeypatch # Keep for potential future mocks
):
    """
    ...
    try:
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST, response.text
        response_data = response.json()
        assert "invalid oauth state" in response_data.get("detail", "").lower() # Check lowercase substring
        mock_supabase_auth.exchange_code_for_session.assert_not_called()
    finally:
        if original_get_supabase:
            app.dependency_overrides[real_get_supabase_client] = original_get_supabase
